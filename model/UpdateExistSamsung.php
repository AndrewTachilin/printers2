...
class UpdateExistSamsung
{
     ...
     *
     */
    protected function issetPrinter($procent, $fabricator, $port)
    {

        $con = DbConnect::getConnect();
        $result = $con->query("SELECT black_toner,port_name.name,port_name.id from BlackToner 
          LEFT JOIN port_name ON BlackToner.name = port_name.id
         where port_name.name='$port'");
        ...
        if ($show) {
            ...
            return true;
                ...
                try {
                    ...
                    $con->exec("SELECT id  FROM NameFabricator WHERE fabricator='hp'");
